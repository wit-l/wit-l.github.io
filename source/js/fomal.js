...
function createtime2() {
  ...
  setTimeout(
    console.warn.bind(
      console,
      "%c ⚡ Powered by Wit-l🐍 %c 你正在访问Wit-l🐍の小家",
...
function share_() {
  ...
  try {
    ...
    var subTitle = title.endsWith("| Wit-l🐍")
...
function createtime() {
  ...
  (currentTimeHtml =
    hnum < 18 && hnum >= 9
      ? `<img class='boardsign' src='https://cdn.jsdelivr.net/gh/wit-l/static_resources@latest/images/ShangBanMoYvZhong.svg' title='什么时候能够实现财富自由呀~'><br> <div style="font-size:13px;font-weight:bold">本站居然运行了 ${dnum} 天 ${hnum} 小时 ${mnum} 分 ${snum} 秒 <i id="heartbeat" class='fas fa-heartbeat'></i> <br> 旅行者 1 号当前距离地球 ${dis} 千米，约为 ${unit} 个天文单位 🚀</div>`
      : `<img class='boardsign' src='https://cdn.jsdelivr.net/gh/wit-l/static_resources@latest/images/XiaBanXiuXiLa.svg' title='下班了就该开开心心地玩耍~'><br> <div style="font-size:13px;font-weight:bold">本站居然运行了 ${dnum} 天 ${hnum} 小时 ${mnum} 分 ${snum} 秒 <i id="heartbeat" class='fas fa-heartbeat'></i> <br> 旅行者 1 号当前距离地球 ${dis} 千米，约为 ${unit} 个天文单位 🚀</div>`),
...
function setTrans() {
  ...
  target.innerHTML = "不透明度 (0%-100%): " + newTransNum + "%";
...
<div class="settings" style="display: block;">
  <div id="article-container" style="padding:12px;">
    <br>
    <center>
      <p><button onclick="reset()" style="background:linear-gradient(to right, #fc354c, #0abfbc);display:block;width:40%;padding:15px 0;border-radius:30px;color:white;font-size:1.1em;"><i class="fa-solid fa-arrows-rotate" style="animation: fa-spin 8s linear infinite;"></i>&nbsp;恢复默认设置</button></p>
    </center>
    <h2>一、显示偏好</h2>
    ...
    <h2>四、背景设置</h2>
    <center><button onclick="resetBg()" style="background:var(--theme-color);display:block;width:35%;padding:15px 0;border-radius:30px;color:white;"><i class="fa-solid fa-arrows-rotate" style="animation: fa-spin 8s linear infinite;"></i>&nbsp;恢复默认背景</button></center>
