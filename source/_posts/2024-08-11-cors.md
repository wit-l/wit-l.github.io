...
&nbsp;&nbsp;&nbsp;&nbsp;下面将在跨域服务器后端使用 express 模块提供的 http/https 服务监听服务器 3000 端口上的路由 /api/users，收到请求后返回携带 `Access-Control-Allow-Headers:*` 标头的响应信息以允许任意源的js代码获取数据。
...
&nbsp;&nbsp;&nbsp;&nbsp;第6行表示允许来自任意源的跨域请求（即添加了标头`Access-Control-Allow-Origin:*`）。第7行表示提供静态文件服务，将public文件夹下的所有文件挂载到域名的根路由上，可用于模拟public下的前端服务和api在同一个源的情况。第8行表示当本服务器收到发往路由/api/users上的请求时，读取data/users.json文件后将内容返回给请求方。
